...
void CInfraredBasics::ProcessColor()
{
    ...
    if (LockedRect.Pitch != 0)
    {
		...
		//jsyeh begin: Use OpenCV to copy the data
		if(imgIR==NULL){
			imgIR = cvCreateImageHeader(cvSize(cColorWidth, cColorHeight), IPL_DEPTH_8U,4);
		}
		imgIR->imageData = (char*)m_pTempColorBuffer;
		cvNamedWindow("imgIR");
		cvShowImage("imgIR", imgIR);
		//jsyeh end
    }
	...
	if(key=='s'){
		static int n=0;
		char filename[100];
		sprintf(filename, "IR%04d.jpg",n);
		FILE * fin=fopen(filename, "w+");
		cvSaveImage(filename, imgIR);
		n++;
		MessageBoxA(NULL, filename, "cvSaveImage", MB_OK);
	}
	//jsyeh end
}
